...
class RucoyBot():
	def __init__(self, mode, bpq, weapon, special, usepots, pottime, serial, ratio, ipaddr):
		...
		with open('1m.txt', 'r') as f:
			self.matrix_1 = [[int(num) for num in line.split(',')] for line in f]
		with open('2m.txt', 'r') as f:
			self.matrix_2 = [[int(num) for num in line.split(',')] for line in f]
		with open('3m.txt', 'r') as f:
			self.matrix_3 = [[int(num) for num in line.split(',')] for line in f]
		with open('4m.txt', 'r') as f:
			self.matrix_4 = [[int(num) for num in line.split(',')] for line in f]
		with open('5m.txt', 'r') as f:
			self.matrix_5 = [[int(num) for num in line.split(',')] for line in f]
	...
	def update_monster_distances(self):
		...
		for monster_id in self.monster_ids:
			...
			start_point = self.grid.node(30, 20)
			end_point = self.grid.node(30+new_x, 20-new_y)
			#self.prettyprint('x: {}, y: {}, mx: {}, my: {}'.format(x,y,monster_x,monster_y))
			path, runs = self.finder.find_path(start_point, end_point, self.grid)
			#self.prettyprint('path: {} runs: {}'.format(path, runs))
			''
			pathstr = self.grid.grid_str(path=path, start=start_point, end=end_point)
			for part in pathstr.split('\n')[::-1]:
				self.prettyprint(part)
			''
			if len(path) == 0:
				monster_distance = 1000
			else:
				#self.prettyprint(path)
				monster_distance = len(path)-1
			self.grid.cleanup()
	...
	def update_coords(self, coords):
		self.coords = coords
		'''
		x, y, z = self.splitandint(coords)
		
		if z == 0:
			coord_matrix = []
			for b in range(y-20, y+20):
				width_list = []
				for a in range(x-30, x+30):
					width_list.append(self.matrix_1[b][a])
			...
			for b in range(y-20, y+20):
				width_list = []
				for a in range(x-30, x+30):
					width_list.append(self.matrix_2[b][a])
				coord_matrix.append(width_list) 
		elif z == 2:
			coord_matrix = []
			for b in range(y-20, y+20):
				width_list = []
				for a in range(x-30, x+30):
					width_list.append(self.matrix_3[b][a])
				coord_matrix.append(width_list) 
		elif z == 3:
			coord_matrix = []
			for b in range(y-20, y+20):
				width_list = []
				for a in range(x-30, x+30):
					width_list.append(self.matrix_4[b][a])
				coord_matrix.append(width_list) 
		else:
			coord_matrix = []
			for b in range(y-20, y+20):
				width_list = []
				for a in range(x-30, x+30):
					width_list.append(self.matrix_5[b][a])
				coord_matrix.append(width_list) 
		
		#self.prettyprint('start')
		#for coord in coord_matrix:
		#	self.prettyprint(coord)
		#self.prettyprint('stop')
		self.grid = Grid(matrix=coord_matrix)
	...
	def monster_slayer(self):
		...
		if xtotap == 0 and ytotap == 0:
			...
			self.prettyprint("Attacking {} at relative {}, {} with a distance of {}.".format(self.monster_ids[self.atktgt].split(':')[0], xtotap, ytotap, self.monster_distances[self.atktgt]))
	...
	def take_action(self):
		...
		if self.next_action == '':
			...
			#self.prettyprint('There are currently {} monsters around.'.format(len(self.monster_distances)))
			if len(self.drop_ids) > 0 and self.mode == 'farm':
				...
				if action_time != 0:
					...
					if self.monster_distances[self.atktgt] > 5:
						...
						self.next_action = 'step1@{}'.format(action_time+3.5)
				else:
			...
			#self.diagnostics()
			if self.special and self.amigoodtospecial() and self.mode == 'skill' and self.atktgt != '' and self.last_special+0.85 <= time.time() and self.weapon == 'mage': 
				self.adb.tapspecial()
				self.last_special = time.time()
				self.move_closer()
				...
				self.tempignore = []
			if self.special and self.amigoodtospecial() and self.mode == 'skill' and self.atktgt != '' and self.last_special+0.3 <= time.time() and self.weapon != 'mage': 
				self.adb.tapspecial()
				self.last_special = time.time()
				for monster in self.tempignore:
					self.try_to_forgive(monster, 'yes')
			...
			if self.special and not self.amiblocked() and self.mode == 'skill': 
				...
				templist = []
				for monster_id in self.monster_distances:
					if self.monster_distances[monster_id] == 1:
						self.tempignore.append(monster_id)
				black_ids = [x.split('@')[0] for x in self.blacklist]
				for monster in self.tempignore:
					if monster not in black_ids:
						self.update_blacklist(monster, True)
				self.next_action = ''
				return
			else:
				for monster in self.tempignore:
			...
			if self.old_coords == self.coords and self.last_attack+3 <= time.time():
				...
				if x == 0 and y == 0:
					x = 1
				self.adb.tapat(x, y)
				#self.prettyprint('Same coordinates and 3 seconds since last attack.')
				self.next_action = ''
				self.update_blacklist(self.atktgt)
			elif self.special and self.mode == 'skill':
				if self.last_attack+3 <= time.time():
					self.next_action = ''
			elif self.plzlist[0].split(':')[0] == 'hit' and self.plzlist[0].split(':')[1] == self.atktgt:
				if self.last_attack+3 <= time.time():
					#self.prettyprint('Last move was a successful request to attack our atktgt.')
					#self.prettyprint('Unfortunately, an attack wasn\'t successful.')
					self.next_action = ''
					if self.weapon != 'melee':
						x = randint(-2, 2)
						y = randint(-2, 2)
				...
				#self.prettyprint('Move/Hit combo substep.')
				'''
				if self.special and self.mode == 'skill':
					pass
				else:
				'''
				if self.weapon == 'melee':
				...
				if self.old_coords != self.coords:
					#self.prettyprint('Step 1 completed, but coords changed, and we didn\'t get')
					#self.prettyprint('our target.')
					self.next_action = ''
	...
	def update_blacklist(self, nogood='forgive', temp=False):
		if nogood == 'forgive':
			...
			if nogood in self.darker_than_blacklist:
				...
				if temp:
					nogood_and_time = '{}@{}'.format(nogood, time.time()+30)
				else:
	...
	def try_to_forgive(self, monster_id, force='no'):
		...
		if monster_id in self.monster_distances:
			...
			if self.plzlist[0].split(':')[1] != monster_id and self.plzlist[1].split(':')[1] != monster_id and self.last_attack+2 <= int(time.time()) and self.last_forgive+2 <= int(time.time()):
				self.next_action = ''
		'''
		if self.mode != 'skill' and self.special != True:
			if self.weapon == 'melee':
				if self.last_attack+1.3 <= time.time() and self.last_forgive+1.3 <= time.time() and self.next_action != '' and self.plzlist[0].split(':')[1] != self.atktgt:
					self.next_action = ''
					self.last_forgive = time.time()
			else:
				if self.last_attack+1.3 <= time.time() and self.last_forgive+1.3 <= time.time() and self.next_action != '' and self.plzlist[0].split(':')[1] != self.atktgt and self.plzlist[1].split(':')[1] != self.atktgt:
					self.next_action = ''
					self.last_forgive = time.time()
	...
	def gimme_the_training(self, slot, id):
		if self.start_time+60 >= time.time():
			if self.mode == 'skill':
				if self.weapon == 'melee':
	...
	def amigoodtospecial(self):
		count = 0
		for monster in self.monster_distances:
			if self.monster_distances[monster] == 1:
				count += 1
		return count >= 1
	...
	def diagnostics(self):
		...
		self.prettyprint(f'next action: {self.next_action}\tlast atk: {self.last_attack}')
