...
def listener(mode, qu, ipaddr, modme, pqueue, botq, botpower, botprintq, resolve_levels, pm_notifications, pm_notify_email, upload_data, region, servnum, all_filters, serial, ratio, atkok):
	...
	with pydivert.WinDivert(wdstr) as w:
		if temp == 1:
			...
			temp = 0
		for packet in w:
			# As of version 1.2.1 of Rucoy Online, all packets have 4 bytes with 
			# the length of the following data.. I have no idea why this was introduced
			# as there were no problems before but my best guess is the developer 
			# updated the library and this is unknown to him.
			...
			if original.find(b'\x08\x00\x03\x00\x01\x00\x03\x00\x01\x00\x03\x00\x01\x00\x03\x00\x01\x00\x03\x00\x01\x00\x03\x00\x01\x00\x03\x00\x01\x00\x03\x00\x01') >= 0:
				packet.tcp.payload = packet.tcp.payload.replace(b'\x08\x00\x03\x00\x01\x00\x03\x00\x01\x00\x03\x00\x01\x00\x03\x00\x01\x00\x03\x00\x01\x00\x03\x00\x01\x00\x03\x00\x01\x00\x03\x00\x01', b'\x09\x00\x03\x00\x01\x00\x03\x00\x01\x00\x03\x00\x01\x00\x03\x00\x01\x00\x03\x00\x01\x00\x03\x00\x01\x00\x03\x00\x01\x00\x03\x00\x01\x00\x03\x00\x01')
				payload_length = int(binascii.hexlify(packet.tcp.payload[:4]), 16)
				payload_length += 4
				packet.tcp.payload = payload_length.to_bytes(4, 'big') + packet.tcp.payload[4:]
			
			w.send(packet)
			
			if packet.tcp.dst_port != 4000:
				if len(packet.tcp.payload) == 4 or packet.tcp.payload[4:] == b'':
					continue
				...
				if len(longpacket) > 0:
					...
					longpacket = b''
				packet.tcp.payload = packet.tcp.payload[4:]
...
item_names = {
	...
	449: "Minotaur Wand",
	452: "Minotaur Shield",
	455: "Training Dagger Atk.4",
	456: "Training Bow Atk.4",
	457: "Training Wand Atk.4",
	...
	try:
		if origin == "client":
			...
			if argument[:1] == b'\x0c': 
				...
				return mystery_six(argument[1:])
			elif argument[:1] == b'\x30':
				return stamina(argument[1:])
			elif argument == b'':
				return ''
			else:
				return str(argument)
...
def mystery_three(skipme):
	try:
		# \x1a This command isn't fully understood yet. It appears to have to
		# do with special graphics like special attacks and changing the course
		# you take along a map (the green line). Further research is needed.
		if skipme[8:] == b'':
			return ""
		else:
			if skipme[:1] == b'\x05':
				if bp != 'monitor':
					bq.put('takeatk|0')
				return "{}".format(translator(skipme[12:], "server"))
			elif skipme[:1] == b'\x06':
				if bp != 'monitor':
					bq.put('takeatk|0')
				...
				extra = int(binascii.hexlify(skipme[6:7]), 16)
				return "{}".format(translator(skipme[8+extra:], "server"))
	except Exception as e:
...
def stamina(howmuch):
	try:
		# not sure what first two bytes of the arguments are but...
		stam = int(binascii.hexlify(howmuch[2:6]), 16)
		timetxt = timetotxt(stam)
		if stam > 0:
			returnable = 'Stamina currently is at {}|'.format(timetxt)
		elif timetxt == 'error':
			returnable = ''
		else:
			returnable = 'Stamina is currently empty.|'
		if howmuch[7:] == b'':
			return returnable
		else:
			return "{}{}".format(returnable, translator(howmuch[7:], "server"))
	except Exception as e:
		return "An unknown error occurred - stamina - {}".format(e)

def timetotxt(seconds):
	try:
		days = seconds // 86400
		left = seconds % 86400
		hours = left // 3600
		...
		if minutes != 0:
			if minutes > 1:
				...
				returnme += '{:,d} minute'.format(minutes)
		...
		if seconds != 0:
			if seconds > 1:
				...
				returnme += '{:,d} second'.format(seconds)
			...
			if whotowho[lastindex:lastindex+1] == b'\x09' and lastindex != 0:
				...
				if monster_id > 65536:
					if victim_id == global_settings['myid'] and monster_id not in global_settings['ignore']:
						...
						if mofo.find('#') >= 0:
							message = "Hurry and fight back! This asshole is attacking you! FIGHT OR DIE!"
						else:
					...
					if victim_id == global_settings['myid'] and monster_id not in global_settings['ignore']:
						global_settings['ignore'].append(monster_id)
						my_name = playerdict('get', global_settings['myid'])
						monster_name = monsterdict("get", monster_id)
						if monster_name.find(attack_me) == -1:
							if my_name.find('#') >= 0:
								my_name = 'idk'
							else:
								my_name = my_name.split(' Lv')[0]
							if monster_name.find('#') >= 0:
								message = "A monster that doesn't match '{}' is attacking you. Please check this hasn't affected botting. Thank you.".format(attack_me)
							else:
								message = "A {} that doesn't match '{}' is attacking you. Please check this hasn't affected botting. Thank you.".format(monster_name, attack_me)
							toastme = "Warning, {}, {}".format(my_name, message)
							toaster.show_toast("Rucoy Bot", toastme, icon_path="robot.ico", duration=5, threaded=True)
							if global_settings['pm_notifications']:
								pq.put('send|{}|{}|{}'.format(global_settings['pm_notify_email'], my_name, toastme))
				...
				if whattoshow[lastindex+4:lastindex+7] == b'\x01\x01\x01':
					...
					if player_num == global_settings['myid']:
						if start_z == 10:
							...
							if z_coord != start_z:
								# Notify user that Z coord has changed ie. stairs were used
								start_z = z_coord
								my_name = playerdict('get', global_settings['myid'])
								if my_name.find('#') >= 0:
									my_name = 'idk'
									...
									my_name = my_name.split(' Lv')[0]
								message = "You have accidentally used the stairs, please check to make sure botting won't be affected."
								if my_name == 'idk':
...
class BotInterim():
	def __init__(self, botq):
		self.bq = botq
		self.sendme = ''
		
	def put(self, message):
		self.sendme += '{}+'.format(message)
	
	def send(self):
		if len(self.sendme) != 0:
			self.sendme = self.sendme[:-1]
			self.bq.put(self.sendme)
			self.sendme = ''
